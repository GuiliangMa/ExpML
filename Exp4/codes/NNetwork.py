...
class NeuralNetwork:
    def __init__(self, layers, activations, loss='mse'):
        ...
        self.loss_func, self.loss_derivative = self._get_loss_function(loss)
    ...
    def backward(self, y_true):
        ...
        for i in reversed(range(len(self.weights))):
            error *= self.activation_derivs[i](self.activations[i + 1])
        ...
        for epoch in range(epochs):
            for i in range(0, X_train.shape[0], batch_size):
                ...
                if (i / batch_size) % 100 == 0:
                    print(
                        f"Epoch {epoch + 1}, Batch {int(i / batch_size)}, Loss = {self.loss_func(y_batch, self.forward(X_batch))}")
